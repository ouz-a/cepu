...
impl Umulh {
    ...
    pub const UMULH: InstDesc = InstDesc {
        ...
        value: 0b1001_1011_1100_0000_0111_1100_0000_0000,
        decode: Self::decode,
    };
}
...
impl SubExtendedReg {
    ...
    pub const fn decode(word: u32) -> Instruction {
        ...
        let rn = get_bits_ct!(word, 5, 5) as u8;
        let rd = get_bits_ct!(word, 0, 5) as u8;
        Instruction::SubExtendedReg(Self { sf, rm, option, imm3, rn, rd })
    }

    pub const SUB_EXTENDED_REG: InstDesc = InstDesc {
        mask: 0b0111_1111_1110_0000_0000_0000_0000_0000,
        value: 0b0100_1011_0010_0000_0000_0000_0000_0000,
        decode: Self::decode,
    };
}
...
impl Rbit {
    pub fn exec(self, cpu: &mut Cpu, _old_pc: u64) {
        ...
        let op = cpu.x_read(self.rn.into(), datasize);
        let res =
            if datasize == 32 { (op as u32).reverse_bits() as u64 } else { op.reverse_bits() };
        cpu.x_write(self.rd.into(), res, datasize == 32);
    }
    ...
    pub const fn decode(word: u32) -> Instruction {
        ...
        let rn = get_bits_ct!(word, 5, 5) as u8;
        let rd = get_bits_ct!(word, 0, 5) as u8;
        Instruction::Rbit(Self { sf, rn, rd })
    }

    pub const RBIT: InstDesc = InstDesc {
        mask: 0b0111_1111_1111_1111_1111_1100_0000_0000,
        value: 0b0101_1010_1100_0000_0000_0000_0000_0000,
        decode: Self::decode,
    };
}
...
impl Sdiv {
    pub fn exec(self, cpu: &mut Cpu, _old_pc: u64) {
        ...
        let dividend = sign_extend(cpu.x_read(self.rn.into(), datasize), datasize) as i64;
        let divisor = sign_extend(cpu.x_read(self.rm.into(), datasize), datasize) as i64;

        let result =
            if divisor == 0 { 0 } else { dividend.checked_div(divisor).unwrap_or(i64::MIN) };
        let res = result as u64;
        cpu.x_write(self.rd.into(), bits_get(res, 0, datasize), datasize == 32);
    }
    ...
    pub fn decode(word: u32) -> Instruction {
        ...
        let rn = get_bits_ct!(word, 5, 5) as u8;
        let rd = get_bits_ct!(word, 0, 5) as u8;
        Instruction::Sdiv(Self { sf, rm, rn, rd })
    }

    pub const SDIV: InstDesc = InstDesc {
        mask: 0b0111_1111_1110_0000_1111_1100_0000_0000,
        value: 0b0001_1010_1100_0000_0000_1100_0000_0000,
        decode: Self::decode,
    };
}
